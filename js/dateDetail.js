(function (win) {
  ...
  /**
   * @param timeStamp 传值时间戳，不传则默认为今日
   * @returns {{year: number, month: number, date: number, week: string, annalsAndLunarCalendar: (string|*), dayDetail: string}}
   */
  var dateDetail = function ($) {
    if ($.timeStamp) {
      dNow = new Date($.timeStamp);
    }
